...
RSpec.describe BackfillArchivedFieldInCommits, :elastic, :sidekiq_inline, feature_category: :global_search do
  let(:version) { 20230724151612 }
  ...
  describe '.completed?' do
    ...
    context 'when commits are missing archived' do
      before do
        set_elasticsearch_migration_to(version_mapping_migration, including: false)
    ...
    context 'when no commits are missing archived' do
      before do
        set_elasticsearch_migration_to(version_mapping_migration, including: true)
