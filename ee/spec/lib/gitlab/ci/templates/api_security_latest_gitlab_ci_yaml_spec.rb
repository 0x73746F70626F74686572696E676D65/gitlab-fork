...
RSpec.describe 'API-Security.latest.gitlab-ci.yml', feature_category: :continuous_integration do
  ...
  describe 'the created pipeline' do
    ...
    context 'when stages includes dast' do
      ...
      context 'when project has Ultimate license' do
        ...
        it_behaves_like 'acts as branch pipeline', %w[api_security]

        it_behaves_like 'acts as MR pipeline', %w[api_security], { 'CHANGELOG.md' => '' }

        context 'when setting APISEC_DISABLED' do
          before do
            create(:ci_variable, project: project, key: 'APISEC_HAR', value: 'testing.har')
            create(:ci_variable, project: project, key: 'APISEC_TARGET_URL', value: 'http://example.com')
          end
          ...
          context 'when APISEC_DISABLED="true"' do
            before do
              create(:ci_variable, project: project, key: 'APISEC_DISABLED', value: 'true')
            end
          ...
          context 'when APISEC_DISABLED="false"' do
            before do
              create(:ci_variable, project: project, key: 'APISEC_DISABLED', value: 'false')
            end
