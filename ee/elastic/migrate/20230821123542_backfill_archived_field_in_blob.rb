...
class BackfillArchivedFieldInBlob < Elastic::Migration
  ...
  def enqueue_tasks_for_projects(projects_in_progress)
    ...
    projects.each do |project|
      ...
      projects_in_progress << { task_id: task_id, project_id: project.id }
      break if projects_in_progress.size >= project_limit
    end

    projects_missing_from_index = project_ids_to_work_on - projects.pluck(:id)
  ...
  def get_failed_or_completed_projects(projects)
    ...
    projects.each do |item|
      ...
      if task_status['error'].present?
        log_warn("Failed to update blobs",
      ...
      if task_status['completed'].present?
        log("Completed: backfill archived in blobs index",
          project_id: project_id,
          search_task_id: task_id
        )
        ...
        log("In Progress: backfill archived in blobs index",
          project_id: project_id,
          search_task_id: task_id
        )
      end
  ...
  def update_by_query(project)
    ...
    query[:bool][:filter] << { term: { project_id: project.id } }
    response = client.update_by_query(
      ...
      body: {
        ...
        script: {
          lang: 'painless',
          source: "ctx._source.archived = #{project.archived}"
        }
      },
      wait_for_completion: false,
      max_docs: batch_size,
      timeout: ELASTIC_TIMEOUT,
      routing: "project_#{project.id}",
      conflicts: 'proceed'
    )
