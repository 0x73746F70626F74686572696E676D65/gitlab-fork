...
class BackfillArchivedFieldInCommits < Elastic::Migration
  ...
  def get_failed_or_completed_projects(projects)
    ...
    projects.each do |item|
      ...
      if task_status['completed'].present?
        log("Completed: backfill archived in commits index",
          project_id: project_id,
          search_task_id: task_id
        )
        ...
        log("In Progress: backfill archived in commits index",
          project_id: project_id,
          search_task_id: task_id
        )
      end
